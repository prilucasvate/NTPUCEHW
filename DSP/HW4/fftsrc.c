...
void fft(Complex *x, int n, int invert){
    ...
    // calculate twiddle factor Wn
    double ang = 2 * PI / n * (invert ? -1 : 1); // FFT * 1, IFFT * -1
    Complex w = {1.0, 0.0}; // initial Wn^0
    Complex wn = {cos(ang), sin(ang)}; // rotate angle

    // butterfly operation and combine
    for (int i = 0; i < n / 2; i++) {
        // t = w * O[i]
        double t_r = w.real * odd[i].real - w.imag * odd[i].imag; // real part
        double t_i = w.real * odd[i].imag + w.imag * odd[i].real; // imag part

        // first half (E[k] + t)
        x[i].real = even[i].real + t_r;
        x[i].imag = even[i].imag + t_i;
        
        // last half (E[k] - t)
        x[i + n/2].real = even[i].real - t_r;
        x[i + n/2].imag = even[i].imag - t_i;

        // update w = w * wn , rotate angle
        double temp_w = w.real * wn.real - w.imag * wn.imag;
