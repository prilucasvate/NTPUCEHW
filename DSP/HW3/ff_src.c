...
void LPF(double *h, int P, double wc) {
    ...
    for (int n = 0; n < P; n++) {
        ...
        // use window function to reduce sinc side-lobe
        // Hamming Window
...
int main(){
    ...
    for(int m=0 ; m < out_samples ; m++){ // each output sample
        ...
        long long base = (long long)m * M + (P - 1) / 2;
        int k_min = (base - (P - 1) + L - 1) / L; // Ceiling division logic roughly
        int k_max = base / L;

        // boundary check
        if (k_min < 0) k_min = 0;
        if (k_max >= in_samples) k_max = in_samples-1;
        // k from k_min to k_max
        for(int k = k_min ; k <= k_max ; k++){ // each input sample
            int index = base - k * L; // filter index adjustment (centered) 0, 512 ,1024
    ...
    FILE *fp_out = fopen("fastest_output.wav", "wb");
